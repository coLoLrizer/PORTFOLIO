...
class BloodProjectile:
    def __init__(self, x, y, tx, ty):
        ...
        self.vx = math.cos(angle) * CONF['BLOOD_PROJ_SPEED'] * 0.8
        self.vy = math.sin(angle) * CONF['BLOOD_PROJ_SPEED'] * 0.8
        self.life = 200
    ...
    def draw(self, surface, cam_offset):
        ...
        end_x = pos[0] - self.vx * 3 * TILE_SIZE; end_y = pos[1] - self.vy * 3 * TILE_SIZE
        pygame.draw.line(surface, (150, 0, 0), pos, (end_x, end_y), 6)
        pygame.draw.circle(surface, (255, 50, 50), pos, 8)
...
class ShadowMinion:
    def __init__(self, x, y, px, py):
        self.x, self.y = x, y; self.hp = CONF['SHADOW_MINION_HP']; self.radius = 12; self.speed = CONF['SHADOW_MINIONS_SPEED']
        self.state = "FOLLOW"; self.timer = 0
        dx, dy = self.x - px, self.y - py; dist = math.hypot(dx, dy)
        if dist < 1.5: push = 2.0; self.x += (dx/dist if dist > 0 else 1) * push; self.y += (dy/dist if dist > 0 else 0) * push
    def update(self, player, game):
        dx, dy = player.x - self.x, player.y - self.y; dist = math.hypot(dx, dy)
        if self.state == "FOLLOW":
            if dist > 1.2: self.x += (dx/dist) * self.speed; self.y += (dy/dist) * self.speed
            else: self.state = "PREPARE"; self.timer = 35
        elif self.state == "PREPARE":
            self.timer -= 1
            if self.timer <= 0:
                if dist < 1.6: player.take_damage(CONF['DMG_SHADOW'], game); game.visual_effects.append(SlashEffect(self, 0, 15, (150, 0, 200), 360, 10, "HIT"))
                self.state = "COOLDOWN"; self.timer = 45 
    ...
    def draw(self, surface, cam_offset):
        ...
        eye_c = (255, 255, 255) if self.state != "PREPARE" else (255, 200, 0)
        pygame.draw.circle(surface, eye_c, (px - 4, py - 4 - z_bounce), 3); pygame.draw.circle(surface, eye_c, (px + 4, py - 4 - z_bounce), 3)
...
class Potion:
    ...
    def on_impact(self, p, g):
        ...
        if self.type == "POISON": 
            r = 45 * self.radius_mult; g.puddles.append(PoisonPuddle(self.x, self.y, radius=r))
            for i in range(3):
                a = math.radians(i * 120); dist = 1.5 * self.radius_mult
                g.puddles.append(PoisonPuddle(self.x + math.cos(a)*dist, self.y + math.sin(a)*dist, radius=r))
            ...
            for i in range(c):
                ...
                g.shadow_minions.append(ShadowMinion(self.x+math.cos(a)*dist, self.y+math.sin(a)*dist, p.x, p.y))
